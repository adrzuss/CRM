...
from services.ventas import get_vta_hoy, get_vta_semana, ventas_por_mes, pagos_hoy, get_operaciones_hoy, get_operaciones_semana, get_ultimas_operaciones, get_10_mas_vendidos, \
                            get_op_este_mes, get_op_este_mes_anterior, get_vta_sucursales_data, get_vta_vendedores_data
...
from utils.utils import check_session, alertas_mensajes, format_currency
...
def tablero_inicial():
    ...
    return render_template('tablero.html', tituloTablero='Gerencia', desde_sucs=desde_sucs, hasta_sucs=hasta_sucs, desde_vend=desde_vend, hasta_vend=hasta_vend, vta_hoy=vta_hoy, vta_semana=vta_semana, saldo_clientes_actual=format_currency(saldo_clientes_actual), saldo_clientes_vencido=format_currency(saldo_clientes_vencido), saldo_proveedores=saldo_proveedores, meses=vta_6_meses['meses'], operaciones=vta_6_meses['operaciones'], tipoPagoss=pagosHoy['tipo_pago'], cantPagoss=pagosHoy['total_pago'], ventasSucursales=ventasSucursales, ventasVendedores=ventasVendedores, alertas=g.alertas, cantidadAlertas=g.cantidadAlertas)    
...
def tablero_administrativo():
    ...
    saldo_proveedores = get_saldo_proveedores()
    pagosHoy = pagos_hoy()
    return render_template('tablero.html', tituloTablero='Administración', desde=desde, hasta=hasta, vta_hoy=vta_hoy, vta_semana=vta_semana, saldo_clientes=saldo_clientes, saldo_proveedores=saldo_proveedores, meses=vta_6_meses['meses'], operaciones=vta_6_meses['operaciones'], tipoPagoss=pagosHoy['tipo_pago'], cantPagoss=pagosHoy['total_pago'], alertas=g.alertas)  
...
def tablero_basico():
    ...
    return render_template('tablero-basico.html', tituloTablero='Básico', op_hoy=op_hoy, op_semana=op_semana, op_este_mes=op_este_mes, detalles=los_10_mas_vendidos['det_arts'], cantidades=los_10_mas_vendidos['vta_arts'], ultimas_op=ultimas_op, stock_negativos=stock_negativos, stock_faltantes=stock_faltantes, alertas=g.alertas, cantidadAlertas=g.cantidadAlertas)