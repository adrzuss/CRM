...
function agregarItemATabla(item, index) {
  ...
  const nuevaFila = `
    <tr class="items">
      ...
      <td><input type="text" class="form-control codigo-articulo" name="items[${index}][codigo]" value="${item.codigo || ''}" readonly></td>
      <td class="descripcion-articulo">${item.descripcion || '-'}</td>
      <td><input type="number" class="form-control precio-unitario" name="items[${index}][precio_unitario]" value="${item.precio_unitario}" readonly></td>
      <td><input type="number" class="form-control cantidad" name="items[${index}][cantidad]" value="${item.cantidad}" step="0.01" min="0.01" required></td>
  ...
  // Agregar event listeners focus (CSP-compatible)
  const nuevaTr = tbody.querySelector(`tr:last-child`);
  nuevaTr.querySelectorAll('.codigo-articulo, .precio-unitario, .cantidad').forEach(input => {
    input.addEventListener('focus', function() { this.select(); });
  });
}
